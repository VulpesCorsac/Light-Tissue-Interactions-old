...
void RTtotal(T a, T tau, T g, T nSlab, T n_slide_top, T n_slide_bottom, const std::array<T,M>& v, const std::array<T,M>& w, Matrix<T,M,M>& Rtotal, Matrix<T,M,M>& Ttotal) {
    ...
    addingBounds<T,M>(Rbound<T,M>(a, tau, g, nSlab, n_slide_top, v, w), myRslab, R20, Tbound<T,M>(a, tau, g, nSlab, n_slide_top, v, w), myTslab, T02);
    addingBounds<T,M>(R20, Rbound<T,M>(a, tau, g, nSlab, n_slide_bottom, v, w), R30, T02, Tbound<T,M>(a, tau, g, nSlab, n_slide_bottom, v, w), Ttotal);
    R30mod = R30 - Rbound<T,M>(a, tau, g, nSlab, n_slide_bottom, v, w);
    for (int i = 0; i < m; i++)
        R30mod(i, i) += Rbound<T,M>(a, tau, g, nSlab, n_slide_bottom, v, w)(i, i) / sqr(twoaw<T,M>(v, w)(i));
    Rtotal = R30mod;
}

/*
template < typename T, size_t M >
Matrix<T,M,M> Ttotal(T a, T tau, T g, T nSlab, T n_slide_top, T n_slide_bottom, const std::array<T,M>& v, const std::array<T,M>& w) {
    Matrix<T,M,M> T02t, R20t, T03t, R30t;
    addingBounds<T,M>(Rbound<T,M>(a, tau, g, nSlab, n_slide_top, v, w), Rslab<T,M>(a, tau, g, nSlab, v, w), R20t, Tbound<T,M>(a, tau, g, nSlab, n_slide_top, v, w), Tslab<T,M>(a, tau, g, nSlab, v, w), T02t);
    addingBounds<T,M>(R20t, Rbound<T,M>(a, tau, g, nSlab, n_slide_bottom, v, w), R30t, T02t, Tbound<T,M>(a, tau, g, nSlab, n_slide_bottom, v, w), T03t);
    return T03t;
}

template < typename T, size_t M >
Matrix<T,M,M> Rtotal(T a, T tau, T g, T nSlab, T n_slide_top, T n_slide_bottom, const std::array<T,M>& v, const std::array<T,M>& w) {
    const int m = M;
    Matrix<T,M,M> T02r, R20r, T03r, R30r, R30mod;
    addingBounds<T,M>(Rbound<T,M>(a, tau, g, nSlab, n_slide_top, v, w), Rslab<T,M>(a, tau, g, nSlab, v, w), R20r, Tbound<T,M>(a, tau, g, nSlab, n_slide_top, v, w), Tslab<T,M>(a, tau, g, nSlab, v, w), T02r);
    addingBounds<T,M>(R20r, Rbound<T,M>(a, tau, g, nSlab, n_slide_bottom, v, w), R30r, T02r, Tbound<T,M>(a, tau, g, nSlab, n_slide_bottom, v, w), T03r);
    R30mod = R30r - Rbound<T,M>(a, tau, g, nSlab, n_slide_bottom, v, w);
    for (int i = 0; i < m; i++)
        R30mod(i, i) += Rbound<T,M>(a, tau, g, nSlab, n_slide_bottom, v, w)(i, i) / sqr(twoaw<T,M>(v, w)(i));
    return R30mod;
}
...
template < typename T, size_t M >
void RTs(T a, T tau, T g, T nSlab, T n_slide_top, T n_slide_bottom, const std::array<T,M>& v, const std::array<T,M>& w, T& Rs, T& Ts) {
    const int m = M;
    Ts = Rs = 0;
...
/*
template < typename T, size_t M >
T Ts(T a, T tau, T g, T nSlab, T n_slide_top, T n_slide_bottom, const std::array<T,M>& v, const std::array<T,M>& w) {
    const int m = M;
    T myTs = 0;
    Matrix<T,M,M> Ttot = Ttotal<T,M> (a, tau, g, nSlab, n_slide_top, n_slide_bottom, v, w);
    for (int i = 0; i < m; i++)
...
template < typename T, size_t M >
T Rs(T a, T tau, T g, T nSlab, T n_slide_top, T n_slide_bottom, const std::array<T,M>& v, const std::array<T,M>& w) {
    const int m = M;
    T myRs = 0;
    Matrix<T,M,M> Rtot = Rtotal<T,M> (a, tau, g, nSlab, n_slide_top, n_slide_bottom, v, w);
    for (int i = 0; i < m; i++)
