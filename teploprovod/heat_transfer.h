...
Matrix<T, Eigen::Dynamic, Eigen::Dynamic> HeatTransfer(T T0, T t_end, T kap_b, T kap_s, T Te, T time, T gamma, T gamma_G, T eps, Matrix<T,Dynamic,Dynamic>& HeatSourceMatrix) {
    ...
    const T L = 0.001, L_s = 0.00, R = 0.01; //начальная темпа, L - длина образца по оси z,  L_s - длина стекла по оси z, R - радиус, температуры на границах.
    constexpr int Nz_s = 0;
    const T hr = R / ((Nr - 1)), hz = (L + 2 * L_s) / Nz, dt = 0.1, tau = 0.05;
    T q1, q2, q3, q4; //тепловые потоки 

    static Matrix<T,Dynamic,Dynamic> Temp;  //поле температуры
    Matrix<T,Dynamic,Dynamic> Tn = Matrix<T, Nz, Nr>::Zero();; //поле температуры на предыдущем временном слое 
    Matrix<T,Dynamic,Dynamic> Ts = Matrix<T, Nz, Nr>::Zero();; //поле температуры на предыдущей иттерации
    static Matrix<T,Dynamic,Dynamic> G; //поле степени деградации ткани G
    Matrix<T,Dynamic,Dynamic> Gn = Matrix<T, Nz, Nr>::Zero();; //поле G на предыдущем временном слое 
    ofstream f;
    ofstream g;   
    f.open("Results/res2dcyl_A_t:_" + to_string(int(time)) + "_gamma_G:_" + to_string(gamma_G) + "_eps:_" + to_string(eps) + ".xls");
    g.open("Results/CoagMatrix_t:_" + to_string(int(time)) + ".csv");
    ...
    if (time == 0) {
        Temp = Matrix<T, Nz, Nr>::Constant(Nz,Nr,T0);
    ...
    while (time < t_end) {
      ...
      for (int j = 0; j <= (Nr - 1); j ++) {
            alfa[0] = 1;
            beta[0] = 0;//поток тепла снизу равен нулю
            ...
            do {
                ...
                //гр. усл 3
                Temp((Nz - 1), j) = (lambda_G(G((Nz - 1), j), Temp((Nz - 1), j), gamma_G) * beta[(Nz - 1) - 1] + hz * kap_b * Te) / (hz * kap_b + lambda_G(G((Nz - 1), j), Temp((Nz - 1), j), gamma_G) * (1 - alfa[(Nz - 1) - 1]));
                
                if (G((Nz - 1), j) < 0.01) {
                    G((Nz - 1), j) = 0;
                }
                else { 
                    G((Nz - 1), j) = Gn((Nz - 1), j) * (1 - K(Temp((Nz - 1), j)) * tau);//определяем поле G
                }

                // Определяем поле температуры и G на промежуточном временном слое(прошли по оси z):
                for (int i = (Nz - 1)- 1; i >= 0; i --) {
       ...
       time += 0.05;//добавляем время, т.к. находимся на промежуточном слое по времени
        ...
        for (int i = 0; i <= (Nz - 1); i ++) {
            ...
            if ((i >= Nz_s) && (i <= Nz_s + (Nz - 1))) { 
                alfa[0]= 1.0;
                beta[0]= pow(hr,2) * HeatSourceMatrix(i - Nz_s, 0) / lambda_G(G(i - Nz_s, 0), Temp(i - Nz_s, 0), gamma_G);
            }
            ...
            do {
                ...
                for (int j = 1; j < (Nr - 1); j ++) {
                    ...
                    if ((i >= Nz_s) && (i <= Nz_s + (Nz - 1))) { 
                        ai = 0.5 * (lambda_G(G(i - Nz_s, j), Temp(i - Nz_s, j), gamma_G) + lambda_G(G(i - Nz_s, j + 1), Temp(i - Nz_s, j + 1), gamma_G)) / pow(hz,2);
                        ci = 0.5 * (lambda_G(G(i - Nz_s, j - 1), Temp(i - Nz_s, j - 1), gamma_G) + lambda_G(G(i - Nz_s, j), Temp(i - Nz_s, j), gamma_G)) / pow(hz,2);
                        bi = ai + ci + ro * c / tau;
                        fi = -ro * c * Tn(i, j) / tau - HeatSourceMatrix(i - Nz_s, j);
                ...
                if ((i >= Nz_s) && (i <= Nz_s + (Nz - 1))) { 
                    // гр усл. 3
                    Temp(i, (Nr - 1))= (lambda_G(G(i - Nz_s, (Nr - 1)), Temp(i - Nz_s, (Nr - 1)), gamma_G) * beta[(Nr - 1) - 1] + hr * kap_b * Te + 0.25 * HeatSourceMatrix(i, (Nr - 1))* pow(hz,2)) / 
                                                                    (hr * kap_b + lambda_G(G(i - Nz_s, (Nr - 1)), Temp(i - Nz_s, (Nr - 1)), gamma_G) * (1 - alfa[(Nr - 1) - 1]));    
                }
