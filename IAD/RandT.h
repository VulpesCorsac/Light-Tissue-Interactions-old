...
void RTslab(T a, T tau, T g, T n_slab, array<T, M> v, array<T, M> w, Matrix<T, M, M>& Rslab, Matrix<T, M, M>& Tslab){
    doubling<T, M>(a, tau, g, n_slab, v, w, Rslab, Tslab);
}

/*template <class T, size_t M>
Matrix<T, M, M> Rslab(T a, T tau, T g, T n_slab, array<T, M> v, array<T, M> w) {
...
template <class T, size_t M>
Matrix<T, M, M> Tslab(T a, T tau, T g, T n_slab, array<T, M> v, array<T, M> w) {
    Matrix<T, M, M> myRslab;
    Matrix<T, M, M> myTslab;
    doubling<T, M>(a, tau, g, n_slab, v, w, myRslab, myTslab);
    return myTslab;
...
template <class T, size_t M>
Matrix<T, M, M> Rbound(T a, T tau, T g, T n_slab, T n_slide, array<T, M> v, array<T, M> w) {
...
template <class T, size_t M>
Matrix<T, M, M> Tbound(T a, T tau, T g, T n_slab, T n_slide, array<T, M> v, array<T, M> w) {
...
template <class T, size_t M>
void RTtotal(T a, T tau, T g, T n_slab, T n_slide_top, T n_slide_bottom, array<T, M> v, array<T, M> w, Matrix<T, M, M>& Rtotal, Matrix<T, M, M>& Ttotal) {
    int m = M;
    ...
    for (int i = 0; i < m; i++)
        R30mod(i, i) += Rbound<T, M>(a, tau, g, n_slab, n_slide_bottom, v, w)(i, i)/pow(twoaw<T, M>(v, w)(i),2);
    Rtotal = R30mod;
}

/*template <class T, size_t M>
Matrix<T, M, M> Ttotal(T a, T tau, T g, T n_slab, T n_slide_top, T n_slide_bottom, array<T, M> v, array<T, M> w) {
...
template <class T, size_t M>
Matrix<T, M, M> Rtotal(T a, T tau, T g, T n_slab, T n_slide_top, T n_slide_bottom, array<T, M> v, array<T, M> w) {
    int m = M;
...
template <class T, size_t M>
void RTs (T a, T tau, T g, T n_slab, T n_slide_top, T n_slide_bottom, array<T, M> v, array<T, M> w, T& Rs, T& Ts) {
    T myTs = 0.0;
    T myRs = 0.0;
    int m = M;
...
/*template <class T, size_t M>
T Ts (T a, T tau, T g, T n_slab, T n_slide_top, T n_slide_bottom, array<T, M> v, array<T, M> w) {
    T myTs = 0.0;
    int m = M;
...
template <class T, size_t M>
T Rs (T a, T tau, T g, T n_slab, T n_slide_top, T n_slide_bottom, array<T, M> v, array<T, M> w) {
    T myRs = 0.0;
    int m = M;
